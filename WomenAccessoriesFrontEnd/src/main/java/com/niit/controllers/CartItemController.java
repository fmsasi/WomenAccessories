...
public class CartItemController {
	...
	public String addToCart(@PathVariable int productId, @RequestParam int requestedQuantity,
			@AuthenticationPrincipal Principal principal)// in jsp
															// userPrincipal, in
															// controller
															// Principal

	{
		...
		for (CartItem cartItem : cartItems) {
			if (cartItem.getProduct().getId() == productId) {
				...
				cartItemDao.addToCart(cartItem);// update the quantity and
												// totalprice
	...
	public String removeCartItem(@PathVariable int cartItemId) {
		...
		// select * from cartitem where user_email=? - to execute the query
